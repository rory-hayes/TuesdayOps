...
- Improved first-run form clarity with shared inline validation for agency onboarding, new client creation, and manual workflow setup, including field-specific errors, `aria-invalid` wiring, stable accessible names, and component coverage.
- Hardened check execution reliability by ignoring stale mismatched decrypted auth config during runs and making manual check feedback status-specific.
...
- Completed an E2E QA/fix pass with Tuesday rebrand copy updates, full-row drilldowns, workflow registry filters, report-list simplification, inline report editing persistence, compact report readiness status, sent-report read-only preservation, and a documented `TUESDAYOPS_E2E_QA_AUDIT.md`.
