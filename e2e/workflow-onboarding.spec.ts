...
test("workflow can be imported from a cURL command", async ({ page, baseURL }) => {
  ...
  await clientDialog.getByLabel("Report email").fill(`workflow-import-${runId}@example.invalid`);
  await clientDialog.getByLabel("Notes").fill("Workflow import E2E client.");
