...
describe("google auth route", () => {
  ...
  it("returns OAuth errors to the originating sign-up page with safe copy", async () => {
    ...
    expect(response.headers.get("location")).toBe(
      "https://app.example.com/sign-up?error=Google%20sign-in%20could%20not%20be%20started.%20Try%20again.",
    );
  });
  ...
  it("defaults invalid source values back to the sign-in page", async () => {
    ...
    expect(response.headers.get("location")).toBe(
      "https://app.example.com/sign-in?error=Google%20sign-in%20could%20not%20be%20started.%20Try%20again.",
    );
  });
});
